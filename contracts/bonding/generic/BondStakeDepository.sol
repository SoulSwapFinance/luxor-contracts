...
contract LuxorBondStakeDepository is Ownable {
    ...
    address public immutable LUX; // intermediate reward token from treasury
    address public immutable LUM; // token given as payment for bond
    ...
    struct Bond {
        ...
        uint luxPayout; // LUX amount at the moment of bond
    ...
    function setVestingHours(uint _hours) external onlyPolicy() {
            ...
            terms.vestingTerm = _hours * 3600; // 60s * 60m = 3_600s
    }
    ...
    function deposit( 
        ...
        _bondInfo[ _depositor ] = Bond({ 
            ...
            luxPayout: _bondInfo[ _depositor ].luxPayout.add( payout ),
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( LUX ).totalSupply().mul( terms.maxPayout ).div( 100_000 );
    }
    ...
    function bondPrice() public view returns ( uint price_ ) {        
        price_ = terms.controlVariable.mul( debtRatio() ).add( 1_000_000_000 ).div( 1e5 );
