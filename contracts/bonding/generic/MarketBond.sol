...
contract MarketBond is Ownable {
    ...
    address immutable luxorDAO;
    address luxorTreasury; // receives fee
    ...
    address immutable subsidyRouter; // pays subsidy in LUX to custom treasury
    ...
    constructor(
        ...
        require( _luxorTreasury != address(0) );
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        ...
        uint fee = payout.mul( currentLuxorFee() ).div( 1e6 );
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            payoutToken.safeTransfer(luxorTreasury, fee);
        }
    ...
    function trueBondPrice() public view returns ( uint price_ ) {
        price_ = bondPrice().add(bondPrice().mul( currentLuxorFee() ).div( 1e6 ) );
    }
    ...
    /**
     *  @notice calculate user's interest due for new bond, accounting for Luxor Fee
     *  @param _value uint
     *  @return uint
     */
    function payoutFor( uint _value ) external view returns ( uint ) {
        ...
        return total.sub(total.mul( currentLuxorFee() ).div( 1e6 ));
    }
    ...
    /**
     *  @notice current fee Luxor takes of each bond
     *  @return currentFee_ uint
     */
    function currentLuxorFee() public view returns( uint currentFee_ ) {
