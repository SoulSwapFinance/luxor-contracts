...
abstract contract Withdrawable is Context, Ownable {
     ...
     * Can only be called by the current owner.
     */
    function transferWithdrawership(address newWithdrawer)
        public
        virtual
        onlyOwner
    {
...
contract SorMaster is Ownable, Withdrawable, ReentrancyGuard, Pausable {
    ...
    ISorToken public immutable sor = 0xeffd4874aca3acd19a24df3281b5cdadd823801a;
    IERC20 public immutable dai = 0x8d11ec38a3eb5e956b052f67da8bdc9bef8abf3e
    IERC20 public immutable luxor = 0x6671e20b83ba463f270c8c75dae57e3cc246cb2b;
    ILuxorRouter public immutable luxorRouter = 0x6b3d631b87fe27af29efec61d2ab8ce4d621ccbf;
    ...
    function setSwapPath(address[] calldata _swapPath) external onlyOwner {
        require(
            _swapPath.length > 1 &&
                _swapPath[0] == address(dai) &&
                _swapPath[_swapPath.length - 1] == address(luxor),
            "invalid swap path"
        );
        swapPath = _swapPath;
    ...
    function setLuxorPermille(uint256 _luxorPermille) external onlyOwner {
        require(_luxorPermille <= 750, "luxorPermille too high!"); // 75% MAX LUXOR COLLATERAL
    ...
    function setMaxStakePerSecond(uint256 _maxStakePerSecond)
        external
        onlyOwner
    {
    ...
    function stake(
        uint256 amount,
        uint256 luxorAmountOutMin,
        uint256 sorAmountOutMin
    ) external nonReentrant whenNotPaused {
        require(
            block.timestamp > lastSecondLuxorPermilleChanged,
            "luxorPermille just changed"
        );
        ...
        require(
            lastSecondDaiStaked <= maxStakePerSecond,
            "maximum stake per second exceeded"
        );
        ...
        sor.mint(address(this), amount);
        uint256 luxorAmount = (amount * luxorPermille) / 1000;
        dai.approve(address(luxorRouter), luxorAmount);
        luxorRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            luxorAmount,
            luxorAmountOutMin,
            swapPath,
            address(this),
        ...
        require(amount >= sorAmountOutMin, "sorAmountOutMin not met");
        sorClaimAmount[msg.sender] = amount;
    ...
    function claimDai(uint256 luxorAmountOutMin, uint256 daiAmountOutMin)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        uint256 daiTransferAmount = (amount *
            (1000 - luxorPermille - treasuryPermille)) / 1000;
        require(
            daiTransferAmount >= daiAmountOutMin,
            "daiAmountOutMin not met"
        );
        uint256 daiTreasuryAmount = (amount * treasuryPermille) / 1000;
        uint256 luxorTransferAmount = (luxor.balanceOf(address(this)) *
            amount) / sor.totalSupply();
        sor.burn(amount);
